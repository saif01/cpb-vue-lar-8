...
class FeedController extends Controller
{
    ...
    public function index(){
        ...
        $allData = BusinessFeed::orderBy($sort_field, $sort_direction)
    ...
    public function store(Request $request){
        ...
        $this->validate($request,[
            'title'      => 'required|string|max:1000|unique:business_feeds',
            'website'    => 'nullable|string|max:1000',
            'name'      => 'required|string|max:100',
            'details'    => 'required|string|max:40000',
        ]);

        $data = new BusinessFeed();
    ...
    public function update(Request $request, $id){
        ...
        //Validate
        $this->validate($request,[
            'title'      => 'required|string|max:1000|unique:business_feeds,title,'.$id,
            'website'    => 'nullable|string|max:1000',
            'name'       => 'required|string|max:100',
            'details'    => 'required|string|max:40000',
        ]);

        $data = BusinessFeed::find($id);
